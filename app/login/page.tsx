  ...
  return (
    <div style={{
      ...
      <svg width="52" height="52" viewBox="0 0 52 52"
        ...
        <circle cx="26" cy="26" r="20" fill="none" stroke="#e8b84b" strokeWidth="4"
          strokeLinecap="round" strokeDasharray="126" strokeDashoffset="96" />
      </svg>
      ...
      <div style={{ display: "flex", gap: 6 }}>
        {[0, 1, 2].map(i => (
          <div key={i} style={{
            ...
            background: i === idx % 3 ? "#e8b84b" : "#1e2a3a",
...
export default function Login() {
  ...
  // ── Auth check — 3s max (fix mobile IP locale)
  useEffect(() => {
      ...
      if (user && user.emailVerified) {
        try {
          ...
          if (snap.exists()) {
            ...
            setShowLoader(true);
            setTimeout(() => {
              window.location.href = data.onboardingCompleted
                ? "/dashboard" : "/welcome";
            }, 1200);
            return;
          }
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      if (err.code === "auth/popup-blocked") {
        setError(lang === "fr" ? "Popup bloqué. Autorise les popups." :
          lang === "es" ? "Popup bloqueado." : "Popup blocked. Allow popups.");
        return;
      }
      setError(lang === "fr" ? "Erreur Google. Réessaie." :
        lang === "es" ? "Error de Google." : "Google error. Try again.");
    } finally {
  ...
  return (
    <div style={container}>
      ...
      <div style={{
        ...
        <div style={{
          ...inputWrap,
          borderColor: !emailValid ? "#ef4444" : "#1e2a3a",
        }}>
        ...
        <div style={{ textAlign: "right", marginBottom: 16, marginTop: -4 }}>
          <span style={linkStyle} onClick={() => window.location.href = "/forgot"}>
            {text.forgot}
          </span>
        </div>
        ...
        <div style={bottomText}>
          ...
          <span style={linkStyle} onClick={() => window.location.href = "/signup"}>
            {text.signup}
          </span>
        </div>
      </div>
...
const googleBtn: CSSProperties = {
  ...
  background: "#1a2438", border: "1px solid #2a3448",
...
const successBox: CSSProperties = {
  ...
  borderRadius: 10, padding: "12px 14px",
  marginBottom: 12, color: "#22c55e", fontSize: 13,
};
