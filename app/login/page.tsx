  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        ...
        if (result?.user && !unsubscribed) {
          setShowLoader(true);
          await resolveAndRedirect(result.user, savedLang);
          return; // ✅ Sortir — ne pas aller à onAuthStateChanged
        }
      } catch (err: any) {
        // auth/null-user ou pas de redirect = normal, continuer
      }

      // ── Étape 2 : vérifier si déjà connecté ──
      const timeout = setTimeout(() => {
        if (!unsubscribed) setCheckingAuth(false);
      }, 3000);
