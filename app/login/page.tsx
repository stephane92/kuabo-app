...
const T = {
  en: {
    ...
    google: "Continue with Google", googleLoading: "Redirecting to Google...", or: "or",
  ...
  fr: {
    ...
    google: "Continuer avec Google", googleLoading: "Redirection vers Google...", or: "ou",
    ...
    sent: "✅ Email de vérification envoyé", resendError: "Erreur lors de l'envoi. Réessaie.",
  ...
  es: {
    ...
    email: "Correo electrónico", pass: "Contraseña", btn: "Iniciar sesión", loading: "Iniciando sesión...",
    google: "Continuar con Google", googleLoading: "Redirigiendo a Google...", or: "o",
    ...
    sent: "✅ Email de verificación enviado", resendError: "Error al enviar. Inténtalo de nuevo.",
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#0b0f1a", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:24, zIndex:9999 }}>
      ...
      <div style={{ display:"flex", gap:6 }}>
        {[0,1,2].map(i => <div key={i} style={{ width:6, height:6, borderRadius:"50%", background:i===idx%3?"#e8b84b":"#1e2a3a", transition:"background 0.3s" }} />)}
      </div>
...
// ── Résoudre et rediriger un user connecté ──────────────
async function resolveAndRedirect(user: any, lang: Lang) {
  try {
...
export default function Login() {
  ...
  // ✅ Tout dans un seul useEffect — ordre garanti
  useEffect(() => {
    ...
    const init = async () => {
      // ── Étape 1 : vérifier si on revient d'un redirect Google ──
      try {
        const result = await getRedirectResult(auth);
        if (result?.user) {
          redirected = true;
          setShowLoader(true);
          await resolveAndRedirect(result.user, savedLang);
          return;
        }
      } catch (err: any) {
        // Pas de redirect result — normal, continuer
        console.log("getRedirectResult:", err?.code || "null");
      }

      if (redirected) return;

      // ── Étape 2 : vérifier si déjà connecté ──
      const timeout = setTimeout(() => setCheckingAuth(false), 4000);

      const unsub = onAuthStateChanged(auth, async user => {
        clearTimeout(timeout);
        if (!user) { setCheckingAuth(false); return; }

        // ✅ Google users n'ont pas forcément emailVerified
        // On accepte si emailVerified OU si providerData contient google.com
        const isGoogle = user.providerData?.some(p => p.providerId === "google.com");
        if (user.emailVerified || isGoogle) {
          if (!redirected) {
            redirected = true;
            setShowLoader(true);
            await resolveAndRedirect(user, savedLang);
          }
          return;
        }
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!cred.user.emailVerified) { setError("verify"); setLoading(false); return; }
      setSuccess(text.successMsg);
      setShowLoader(true);
      await resolveAndRedirect(cred.user, lang);
    } catch (err: any) {
      switch (err.code) {
  ...
  // ── Login Google — toujours redirect ───────────────────
  const handleGoogle = async () => {
    ...
    try {
      ...
      // La page recharge — getRedirectResult capte le résultat au retour
    } catch (err: any) {
  ...
  return (
    <div style={container}>
      ...
      <div style={{ ...card, opacity:mounted?1:0, transform:mounted?"translateY(0)":"translateY(24px)", transition:"all 0.5s ease" }}>
        <div style={iconCircle}><LogIn size={26} color="#e8b84b" /></div>
        <h2 style={titleStyle}>{text.title}</h2>
        <p style={subStyle}>{text.sub}</p>

        {/* Google */}
        <button style={{ ...googleBtn, opacity:googleLoading?0.7:1 }} onClick={handleGoogle} disabled={loading||googleLoading}>
          {googleLoading ? (
        ...
        <div style={separator}><div style={sepLine} /><span style={sepText}>{text.or}</span><div style={sepLine} /></div>

        {/* Email */}
        <div style={{ ...inputWrap, borderColor:!emailValid?"#ef4444":"#1e2a3a" }}>
          <Mail size={16} color={!emailValid?"#ef4444":"#555"} />
          <input placeholder={text.email} style={inputStyle} value={email} type="email" autoComplete="email"
            onChange={e => setEmail(e.target.value)} onBlur={() => setEmailTouched(true)}
            onKeyDown={e => e.key==="Enter" && handleLogin()} />
          {emailTouched && email && <span style={{ fontSize:14, flexShrink:0 }}>{isValidEmail(email)?"✅":"❌"}</span>}
        </div>
        ...
        <div style={inputWrap}>
          ...
          <input type={showPass?"text":"password"} placeholder={text.pass} style={inputStyle} value={password}
        ...
        {error==="verify" && <div ref={errorRef} style={errorBox}>⚠️ {text.verifyText}<span style={linkStyle} onClick={resendEmail}>{text.resend}</span></div>}
        {error && error!=="verify" && <div ref={errorRef} style={errorBox}>⚠️ {error}</div>}
        {success && <div ref={successRef} style={successBox}>{success}</div>}

        <button style={{ ...loginBtn, opacity:loading?0.8:1 }} onClick={handleLogin} disabled={loading||googleLoading}>
          {loading ? <><Loader2 size={16} style={{ animation:"spin 1s linear infinite", marginRight:8 }} />{text.loading}</> : text.btn}
        </button>

        <div style={{ textAlign:"center", marginTop:16, fontSize:11, color:"#555", lineHeight:1.6 }}>
          {text.terms}{" "}
          <span style={{ color:"#e8b84b", cursor:"pointer", textDecoration:"underline" }} onClick={() => router.push("/terms")}>{text.termsLink}</span>
          {" "}{text.and}{" "}
          <span style={{ color:"#e8b84b", cursor:"pointer", textDecoration:"underline" }} onClick={() => router.push("/privacy")}>{text.privacyLink}</span>
        </div>
