...
// ✅ SEUL CET UID PEUT ACCÉDER
const ADMIN_UID = "fptoylpRLza7V9m5nyNkouC3Qf32";

// ✅ Mot de passe admin haché simple (SHA-256 simulé via btoa)
...
function AdminLogin({ onSuccess }: { onSuccess: () => void }) {
  const [step,       setStep]       = useState<"password"|"setup"|"questions">("password");
  ...
  const handleLogin = async () => {
    if (blocked) return;
    if (!pwd) { setError("Entre ton mot de passe"); return; }
    setLoading(true);
    try {
      const hashed = hashPassword(pwd);
      if (configData?.passwordHash === hashed) {
        // ✅ Succès — sauvegarder session 24h
        ...
        if (newAttempts >= 3) {
          ...
          localStorage.setItem("admin_blocked_until", String(until));
          setBlocked(true);
          setError("❌ 3 tentatives échouées — bloqué 1 heure");
        } else {
          setError(`❌ Mot de passe incorrect (${3 - newAttempts} tentatives restantes)`);
        }
      }
  ...
  const handleSetup = async () => {
    if (!newPwd || newPwd.length < 8) { setError("Mot de passe minimum 8 caractères"); return; }
  ...
  const handleForgot = async () => {
    ...
    try {
      ...
      const d = snap.data();
      if (hashPassword(forgotA1.toLowerCase().trim()) !== d.a1Hash ||
          hashPassword(forgotA2.toLowerCase().trim()) !== d.a2Hash) {
        setError("❌ Réponses incorrectes");
        setLoading(false);
        return;
      }
      if (!forgotPwd || forgotPwd.length < 8) { setError("Nouveau mot de passe min 8 caractères"); setLoading(false); return; }
    ...
    <div style={{ minHeight:"100dvh", background:"#0b0f1a", display:"flex", alignItems:"center", justifyContent:"center", padding:"0 20px" }}>
      <div style={{ width:"100%", maxWidth:400 }}>
        ...
        <div style={{ background:"#0f1521", border:"1px solid #1e2a3a", borderRadius:20, padding:"28px 22px" }}>
          ...
          {step === "setup" && (
            <>
              <div style={{ fontSize:16, fontWeight:800, color:"#f4f1ec", marginBottom:4 }}>
                🛡️ Configuration initiale
              </div>
              <div style={{ fontSize:12, color:"#aaa", marginBottom:20, lineHeight:1.6 }}>
                Première connexion — crée ton mot de passe admin et tes questions de sécurité.
              </div>

              <div style={{ marginBottom:12 }}>
                <div style={{ fontSize:11, color:"#555", marginBottom:6 }}>MOT DE PASSE ADMIN (min 8 caractères)</div>
                <input type="password" value={newPwd} onChange={e=>setNewPwd(e.target.value)}
                  placeholder="••••••••"
                  style={{ width:"100%", padding:"13px", background:"#141d2e", border:"1px solid #1e2a3a", borderRadius:11, color:"#f4f1ec", fontSize:16, fontFamily:"inherit", outline:"none", boxSizing:"border-box" as const }} />
              </div>

              <div style={{ marginBottom:20 }}>
                <div style={{ fontSize:11, color:"#555", marginBottom:6 }}>CONFIRMER LE MOT DE PASSE</div>
                <input type="password" value={confirmPwd} onChange={e=>setConfirmPwd(e.target.value)}
                  placeholder="••••••••"
                  style={{ width:"100%", padding:"13px", background:"#141d2e", border:"1px solid #1e2a3a", borderRadius:11, color:"#f4f1ec", fontSize:16, fontFamily:"inherit", outline:"none", boxSizing:"border-box" as const }} />
              </div>

              <div style={{ fontSize:11, color:"#e8b84b", fontWeight:700, letterSpacing:".08em", textTransform:"uppercase" as const, marginBottom:12 }}>
                Questions de sécurité
              </div>

              <div style={{ marginBottom:10 }}>
                <input value={q1} onChange={e=>setQ1(e.target.value)}
                  placeholder="Question 1 (ex: Nom de ton premier ami ?)"
                  style={{ width:"100%", padding:"11px", background:"#141d2e", border:"1px solid #1e2a3a", borderRadius:10, color:"#f4f1ec", fontSize:14, fontFamily:"inherit", outline:"none", boxSizing:"border-box" as const, marginBottom:6 }} />
                <input value={a1} onChange={e=>setA1(e.target.value)}
                  placeholder="Réponse 1"
                  style={{ width:"100%", padding:"11px", background:"#141d2e", border:"1px solid rgba(232,184,75,.3)", borderRadius:10, color:"#f4f1ec", fontSize:14, fontFamily:"inherit", outline:"none", boxSizing:"border-box" as const }} />
              </div>

              <div style={{ marginBottom:20 }}>
                <input value={q2} onChange={e=>setQ2(e.target.value)}
                  placeholder="Question 2 (ex: Ville où tu es né ?)"
                  style={{ width:"100%", padding:"11px", background:"#141d2e", border:"1px solid #1e2a3a", borderRadius:10, color:"#f4f1ec", fontSize:14, fontFamily:"inherit", outline:"none", boxSizing:"border-box" as const, marginBottom:6 }} />
              ...
              {error && <div style={{ fontSize:12, color:"#ef4444", marginBottom:12, textAlign:"center" as const }}>{error}</div>}

              <button onClick={handleSetup} disabled={loading}
                style={{ width:"100%", padding:"14px", background:"#e8b84b", border:"none", borderRadius:12, color:"#000", fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"inherit", opacity:loading?.7:1 }}>
                {loading ? "⏳..." : "🔐 Créer mon accès admin"}
              </button>
            </>
          )}
          ...
          {step === "password" && !showForgot && (
            <>
              <div style={{ fontSize:16, fontWeight:800, color:"#f4f1ec", marginBottom:20 }}>
                🔐 Connexion Admin
              </div>
              ...
              {blocked ? (
                <div style={{ background:"rgba(239,68,68,.08)", border:"1px solid rgba(239,68,68,.25)", borderRadius:12, padding:"16px", textAlign:"center" as const }}>
                  <div style={{ fontSize:13, color:"#ef4444", lineHeight:1.6 }}>
                    ❌ Trop de tentatives.<br/>Compte bloqué 1 heure.
                  </div>
                </div>
              ) : (
                <>
                  <input
                    ...
                    autoFocus
                    style={{ width:"100%", padding:"14px", background:"#141d2e", border:"1px solid #1e2a3a", borderRadius:11, color:"#f4f1ec", fontSize:16, fontFamily:"inherit", outline:"none", boxSizing:"border-box" as const, marginBottom:12 }}
                  />
                  {error && <div style={{ fontSize:12, color:"#ef4444", marginBottom:12, textAlign:"center" as const }}>{error}</div>}
                  <button onClick={handleLogin} disabled={loading}
                    style={{ width:"100%", padding:"14px", background:"#e8b84b", border:"none", borderRadius:12, color:"#000", fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"inherit", marginBottom:12, opacity:loading?.7:1 }}>
                    {loading ? "⏳..." : "Entrer →"}
                  </button>
                  <button onClick={() => { setShowForgot(true); setError(""); }}
                    style={{ width:"100%", padding:"10px", background:"transparent", border:"none", color:"#555", fontSize:12, cursor:"pointer", fontFamily:"inherit" }}>
                    Mot de passe oublié ?
                  </button>
                </>
              )}
          ...
          {step === "password" && showForgot && configData && (
            <>
              <div style={{ fontSize:16, fontWeight:800, color:"#f4f1ec", marginBottom:6 }}>
                🔑 Récupération
              </div>
              <div style={{ fontSize:12, color:"#aaa", marginBottom:20 }}>Réponds à tes questions de sécurité</div>

              <div style={{ marginBottom:12 }}>
                <div style={{ fontSize:12, color:"#e8b84b", marginBottom:6 }}>{configData.q1}</div>
                <input value={forgotA1} onChange={e=>setForgotA1(e.target.value)}
                  placeholder="Ta réponse"
                  style={{ width:"100%", padding:"12px", background:"#141d2e", border:"1px solid #1e2a3a", borderRadius:10, color:"#f4f1ec", fontSize:14, fontFamily:"inherit", outline:"none", boxSizing:"border-box" as const }} />
              </div>
              <div style={{ marginBottom:12 }}>
                <div style={{ fontSize:12, color:"#e8b84b", marginBottom:6 }}>{configData.q2}</div>
                <input value={forgotA2} onChange={e=>setForgotA2(e.target.value)}
                  placeholder="Ta réponse"
                  style={{ width:"100%", padding:"12px", background:"#141d2e", border:"1px solid #1e2a3a", borderRadius:10, color:"#f4f1ec", fontSize:14, fontFamily:"inherit", outline:"none", boxSizing:"border-box" as const }} />
              </div>
              <div style={{ marginBottom:16 }}>
                <div style={{ fontSize:11, color:"#555", marginBottom:6 }}>NOUVEAU MOT DE PASSE</div>
                <input type="password" value={forgotPwd} onChange={e=>setForgotPwd(e.target.value)}
                  placeholder="••••••••"
                  style={{ width:"100%", padding:"12px", background:"#141d2e", border:"1px solid #1e2a3a", borderRadius:10, color:"#f4f1ec", fontSize:16, fontFamily:"inherit", outline:"none", boxSizing:"border-box" as const }} />
              </div>
              ...
              <div style={{ display:"flex", gap:10 }}>
                ...
                <button onClick={handleForgot} disabled={loading}
                  style={{ flex:2, padding:"12px", background:"#e8b84b", border:"none", borderRadius:11, color:"#000", fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>
        ...
        <div style={{ textAlign:"center" as const, fontSize:11, color:"#333", marginTop:16 }}>
          Kuabo Admin · Accès restreint
        </div>
      </div>
    </div>
  );
...
function AdminDashboard({ onLogout }: { onLogout: () => void }) {
  ...
  const loadAll = async () => {
    try {
      ...
      setStats({ users:usersSnap.size, newThisWeek:newCount, topState, confirmed });
      ...
      msgSnap.forEach(d => {
        const data = d.data() as any;
        if (data.type !== "pub" && data.type !== "event") msgs.push({ id:d.id, ...data });
      });
      msgs.sort((a,b) => new Date(b.publishedAt||0).getTime() - new Date(a.publishedAt||0).getTime());
      setMessages(msgs);

      // Pubs
      const pubSnap = await getDocs(collection(db, "admin_messages"));
      const pubList: any[] = [];
      pubSnap.forEach(d => {
        const data = d.data() as any;
        if (data.type === "pub") pubList.push({ id:d.id, ...data });
      });
      setPubs(pubList);

      // Events
      const evtSnap = await getDocs(collection(db, "admin_messages"));
      const evtList: any[] = [];
      evtSnap.forEach(d => {
        const data = d.data() as any;
        if (data.type === "event") evtList.push({ id:d.id, ...data });
      });
      evtList.sort((a,b) => new Date(a.eventDate||0).getTime() - new Date(b.eventDate||0).getTime());
      setEvents(evtList);
    } catch {}
  };
  ...
  const publishMessage = async () => {
    ...
    try {
      ...
      setMsgTitle({ fr:"", en:"", es:"" });
      setMsgContent({ fr:"", en:"", es:"" });
      setMsgSuccess(true);
  ...
  const Section = ({ title }: { title:string }) => (
    <div style={{ fontSize:10,color:"#555",letterSpacing:".12em",textTransform:"uppercase" as const,marginBottom:8,marginTop:20,fontWeight:600 }}>{title}</div>
  );
    ...
    <div style={{ display:"flex", gap:6, marginBottom:12 }}>
      {(["fr","en","es"] as Lang[]).map(l=>(
        <button key={l} onClick={()=>setLang(l)}
          style={{ padding:"5px 12px", borderRadius:8, background:lang===l?"#e8b84b":"#141d2e", border:"1px solid "+(lang===l?"#e8b84b":"#1e2a3a"), color:lang===l?"#000":"#aaa", fontSize:11, fontWeight:lang===l?700:400, cursor:"pointer", fontFamily:"inherit" }}>
          {l==="fr"?"🇫🇷 FR":l==="en"?"🇺🇸 EN":"🇪🇸 ES"}
        </button>
      ))}
    </div>
      ...
      <div style={{ background:"#0f1521", borderBottom:"1px solid #1e2a3a", padding:"14px 20px", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:100 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <button onClick={onLogout}
            style={{ padding:"6px 12px", background:"rgba(239,68,68,.1)", border:"1px solid rgba(239,68,68,.25)", borderRadius:8, color:"#ef4444", fontSize:12, cursor:"pointer", fontFamily:"inherit" }}>
            Déconnexion
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth:680, margin:"0 auto", padding:"20px 16px" }}>
        ...
        {activeTab === "messages" && (
          <>
            ...
            <div style={{ fontSize:12, color:"#aaa", marginBottom:16 }}>Messages info ou urgents envoyés aux users</div>
            ...
            <div style={{ background:"#141d2e", border:"1px solid #1e2a3a", borderRadius:14, padding:"18px", marginBottom:20 }}>
              ...
              <input value={msgTitle[msgLang]} onChange={e=>setMsgTitle(p=>({...p,[msgLang]:e.target.value}))}
                placeholder={`Titre (${msgLang})`} style={inputStyle}/>
              <textarea value={msgContent[msgLang]} onChange={e=>setMsgContent(p=>({...p,[msgLang]:e.target.value}))}
                placeholder={`Contenu (${msgLang})`} style={textareaStyle}/>

              <Section title="Type"/>
              <div style={{ display:"flex", gap:8, marginBottom:12 }}>
                {([["info","💡 Info","#e8b84b"],["urgent","⚠️ Urgent","#ef4444"]] as const).map(([val,lbl,color])=>(
                  <button key={val} onClick={()=>setMsgType(val)}
                    style={{ flex:1, padding:"10px", background:msgType===val?`${color}15`:"#0f1521", border:`1.5px solid ${msgType===val?color:"#1e2a3a"}`, borderRadius:10, color:msgType===val?color:"#555", fontSize:13, fontWeight:msgType===val?700:400, cursor:"pointer", fontFamily:"inherit" }}>
                    {lbl}
                  </button>
                ))}
              </div>
              ...
              <select value={msgTarget} onChange={e=>setMsgTarget(e.target.value)}
                style={{ ...inputStyle, color:"#f4f1ec" }}>
                {TARGET_OPTS.map(o=><option key={o.value} value={o.value}>{o.label}</option>)}
              </select>

              {msgSuccess && <div style={{ fontSize:13, color:"#22c55e", textAlign:"center" as const, marginBottom:8 }}>✅ Message publié !</div>}

              <button onClick={publishMessage} disabled={savingMsg||!msgTitle.fr}
                style={{ width:"100%", padding:"13px", background:msgTitle.fr?"#e8b84b":"#1e2a3a", border:"none", borderRadius:11, color:msgTitle.fr?"#000":"#555", fontSize:14, fontWeight:700, cursor:msgTitle.fr?"pointer":"default", fontFamily:"inherit", opacity:savingMsg?.7:1 }}>
                {savingMsg?"⏳...":"📢 Publier le message"}
              </button>
            </div>
            ...
            <div style={{ display:"flex", flexDirection:"column" as const, gap:8 }}>
                ...
                <div key={msg.id} style={{ background:"#141d2e", border:`1px solid ${msg.type==="urgent"?"rgba(239,68,68,.25)":"rgba(232,184,75,.2)"}`, borderRadius:12, padding:"14px" }}>
                  <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:12 }}>
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontSize:10, color:msg.type==="urgent"?"#ef4444":"#e8b84b", fontWeight:700, marginBottom:4 }}>
                        {msg.type==="urgent"?"⚠️ URGENT":"💡 INFO"} · {msg.target}
                      </div>
                    ...
                    <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                      <button onClick={()=>toggleActive(msg.id,msg.active)}
                        style={{ padding:"6px 10px", background:msg.active?"rgba(34,197,94,.1)":"rgba(239,68,68,.1)", border:`1px solid ${msg.active?"rgba(34,197,94,.3)":"rgba(239,68,68,.3)"}`, borderRadius:8, color:msg.active?"#22c55e":"#ef4444", fontSize:11, cursor:"pointer", fontFamily:"inherit" }}>
                        {msg.active?"ON":"OFF"}
                      </button>
                      <button onClick={()=>deleteItem(msg.id)}
                        style={{ padding:"6px 10px", background:"rgba(239,68,68,.08)", border:"1px solid rgba(239,68,68,.2)", borderRadius:8, color:"#ef4444", fontSize:11, cursor:"pointer", fontFamily:"inherit" }}>
                        🗑️
                      </button>
                    </div>
                  </div>
              ...
              {messages.length===0&&<div style={{ textAlign:"center" as const, padding:"24px", color:"#555", fontSize:13 }}>Aucun message publié</div>}
            </div>
          </>
        )}
        ...
        {activeTab === "pubs" && (
          <>
            ...
            <div style={{ fontSize:12, color:"#aaa", marginBottom:16 }}>Badge "Partenaire Kuabo" automatique (obligation FTC)</div>
            ...
            <div style={{ background:"#141d2e", border:"1px solid #1e2a3a", borderRadius:14, padding:"18px", marginBottom:20 }}>
              ...
              <input value={pubTitle[pubLang]} onChange={e=>setPubTitle(p=>({...p,[pubLang]:e.target.value}))}
                placeholder={`Titre partenaire (${pubLang})`} style={inputStyle}/>
              <input value={pubDesc[pubLang]} onChange={e=>setPubDesc(p=>({...p,[pubLang]:e.target.value}))}
                placeholder={`Description courte (${pubLang})`} style={inputStyle}/>
              <input value={pubCta[pubLang]} onChange={e=>setPubCta(p=>({...p,[pubLang]:e.target.value}))}
              ...
              <button onClick={publishPub} disabled={savingPub||!pubTitle.fr}
                style={{ width:"100%", padding:"13px", background:pubTitle.fr?"#e8b84b":"#1e2a3a", border:"none", borderRadius:11, color:pubTitle.fr?"#000":"#555", fontSize:14, fontWeight:700, cursor:pubTitle.fr?"pointer":"default", fontFamily:"inherit", opacity:savingPub?.7:1 }}>
            ...
            {/* Liste pubs */}
            <div style={{ fontSize:11,color:"#555",letterSpacing:".08em",textTransform:"uppercase" as const,marginBottom:10,fontWeight:600 }}>
              Pubs actives ({pubs.length})
            </div>
            <div style={{ display:"flex", flexDirection:"column" as const, gap:8 }}>
              {pubs.map(pub=>(
                <div key={pub.id} style={{ background:"#141d2e", border:"1px solid rgba(232,184,75,.2)", borderRadius:12, padding:"14px" }}>
                  <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:12 }}>
                    <div style={{ flex:1 }}>
                    ...
                    <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                      <button onClick={()=>toggleActive(pub.id,pub.active)}
                        style={{ padding:"6px 10px", background:pub.active?"rgba(34,197,94,.1)":"rgba(239,68,68,.1)", border:`1px solid ${pub.active?"rgba(34,197,94,.3)":"rgba(239,68,68,.3)"}`, borderRadius:8, color:pub.active?"#22c55e":"#ef4444", fontSize:11, cursor:"pointer", fontFamily:"inherit" }}>
                        {pub.active?"ON":"OFF"}
                      </button>
                      <button onClick={()=>deleteItem(pub.id)}
                        style={{ padding:"6px 10px", background:"rgba(239,68,68,.08)", border:"1px solid rgba(239,68,68,.2)", borderRadius:8, color:"#ef4444", fontSize:11, cursor:"pointer", fontFamily:"inherit" }}>
                        🗑️
                      </button>
                    </div>
              ...
              {pubs.length===0&&<div style={{ textAlign:"center" as const,padding:"24px",color:"#555",fontSize:13 }}>Aucune pub publiée</div>}
            </div>
          </>
        )}
        ...
        {activeTab === "events" && (
          <>
            ...
            <div style={{ fontSize:12, color:"#aaa", marginBottom:16 }}>Événements Kuabo avec bouton "Participer"</div>
            ...
            <div style={{ background:"#141d2e", border:"1px solid #1e2a3a", borderRadius:14, padding:"18px", marginBottom:20 }}>
              ...
              <button onClick={publishEvent} disabled={savingEvt||!evtTitle.fr||!evtDate}
                style={{ width:"100%", padding:"13px", background:(evtTitle.fr&&evtDate)?"#e8b84b":"#1e2a3a", border:"none", borderRadius:11, color:(evtTitle.fr&&evtDate)?"#000":"#555", fontSize:14, fontWeight:700, cursor:(evtTitle.fr&&evtDate)?"pointer":"default", fontFamily:"inherit", opacity:savingEvt?.7:1 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column" as const, gap:8 }}>
              {events.map(evt=>(
                <div key={evt.id} style={{ background:"#141d2e", border:"1px solid rgba(45,212,191,.2)", borderRadius:12, padding:"14px" }}>
                  <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:12 }}>
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:10, color:"#2dd4bf", fontWeight:700, marginBottom:4 }}>
                        📅 {evt.eventDate}{evt.eventTime?` · ${evt.eventTime}`:""} · {evt.target}
                      </div>
                    ...
                    <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                      <button onClick={()=>toggleActive(evt.id,evt.active)}
                        style={{ padding:"6px 10px", background:evt.active?"rgba(34,197,94,.1)":"rgba(239,68,68,.1)", border:`1px solid ${evt.active?"rgba(34,197,94,.3)":"rgba(239,68,68,.3)"}`, borderRadius:8, color:evt.active?"#22c55e":"#ef4444", fontSize:11, cursor:"pointer", fontFamily:"inherit" }}>
                        {evt.active?"ON":"OFF"}
                      </button>
                      <button onClick={()=>deleteItem(evt.id)}
                        style={{ padding:"6px 10px", background:"rgba(239,68,68,.08)", border:"1px solid rgba(239,68,68,.2)", borderRadius:8, color:"#ef4444", fontSize:11, cursor:"pointer", fontFamily:"inherit" }}>
                        🗑️
                      </button>
                    </div>
              ...
              {events.length===0&&<div style={{ textAlign:"center" as const,padding:"24px",color:"#555",fontSize:13 }}>Aucun événement publié</div>}
            </div>
          </>
        )}
        ...
        {([
          ...
          return (
            <button key={id} onClick={()=>setActiveTab(id)}
              style={{ flex:1, display:"flex", flexDirection:"column" as const, alignItems:"center", justifyContent:"center", gap:3, padding:"10px 0", background:"transparent", border:"none", cursor:"pointer", fontFamily:"inherit", position:"relative" }}>
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, user => {
      ...
      // ✅ Couche 2 : vérifier session admin (expire 24h)
      const session = localStorage.getItem("admin_session");
      const expiry  = localStorage.getItem("admin_session_expiry");
      if (session === "valid" && expiry && Date.now() < parseInt(expiry)) {
        setIsLoggedIn(true);
      }
      setAuthChecked(true);
    });
