    ...
    try {
      ...
      if (user) {
        await updateDoc(doc(db,"users",user.uid), {
          ...
          deadlines: {
            ssn:       new Date(new Date(dateValue).getTime() + 10*86400000).toISOString(),
            bank:      new Date(new Date(dateValue).getTime() + 14*86400000).toISOString(),
            greencard: new Date(new Date(dateValue).getTime() + 21*86400000).toISOString(),
            housing:   new Date(new Date(dateValue).getTime() + 30*86400000).toISOString(),
            license:   new Date(new Date(dateValue).getTime() + 45*86400000).toISOString(),
            job:       new Date(new Date(dateValue).getTime() + 90*86400000).toISOString(),
          }
        });
      }
    } catch {/* continue */}
    ...
    <div style={container}>
      ...
      <div style={{flex:1,display:"flex",justifyContent:"center",alignItems:"center",padding:"88px 20px 40px"}}>
        <div style={{
          ...
          {/* ✅ Bouton continuer UNIQUEMENT — pas de bouton passer */}
          <button
            onClick={handleNext}
