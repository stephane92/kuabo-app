...
const UI: Record<Lang, any> = {
  fr: {
    ...
    required:      "⚠️ Cette information est nécessaire pour calculer tes deadlines",
  },
  en: {
    step:          "Step 4 of 5",
    back:          "Back",
    ...
    required:      "⚠️ This information is needed to calculate your deadlines",
  },
  es: {
    step:          "Paso 4 de 5",
    back:          "Atrás",
    ...
    required:      "⚠️ Esta información es necesaria para calcular tus fechas límite",
  },
};

// ✅ Fix timezone — T12:00:00 évite le décalage UTC
...
export default function Step3b() {
  ...
  // ✅ Fix timezone
  const arrivalDate = dateValue ? new Date(dateValue + "T12:00:00") : null;
  const deadlines = arrivalDate ? {
    ssn:  addDays(dateValue, 10),
    bank: addDays(dateValue, 14),
    gc:   addDays(dateValue, 21),
  } : null;
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      const user = auth.currentUser;
      if (user) {
        // ✅ Fix timezone dans les deadlines Firebase
        const base = new Date(dateValue + "T12:00:00").getTime();
        await updateDoc(doc(db, "users", user.uid), {
          arrivalDate: dateValue,
          deadlines: {
            ssn:       new Date(base + 10 * 86400000).toISOString(),
            bank:      new Date(base + 14 * 86400000).toISOString(),
            greencard: new Date(base + 21 * 86400000).toISOString(),
            housing:   new Date(base + 30 * 86400000).toISOString(),
            license:   new Date(base + 45 * 86400000).toISOString(),
            job:       new Date(base + 90 * 86400000).toISOString(),
          }
        });
      }
    } catch { /* continue */ }
    ...
    <div style={container}>
      ...
      <div style={{ flex: 1, display: "flex", justifyContent: "center", alignItems: "center", padding: "88px 20px 40px" }}>
        <div style={{
          ...
          {/* Message obligatoire */}
          <div style={{ background: "rgba(239,68,68,0.05)", border: "1px solid rgba(239,68,68,0.15)", borderRadius: 10, padding: "10px 14px", marginBottom: 16, fontSize: 12, color: "#ef4444", lineHeight: 1.5 }}>
            {text.required}
          </div>
          ...
          <button
            ...
            style={{ ...nextBtn, opacity: dateValue && !saving ? 1 : 0.4 }}
          >
            {saving ? (
