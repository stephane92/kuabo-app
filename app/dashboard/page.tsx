...
const FULLY_SETTLED: Record<Lang,string> = {
  fr:"🏆 Fully Settled", en:"🏆 Fully Settled", es:"🏆 Instalado"
};
...
function ProfileTab({ userName, userEmail, userCountry, userState, userCity, lang, progress, doneCount, totalSteps, completedSteps, changeLang, onLogout, onDeleteAccount }: {
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      navigator.share({ title:"Kuabo", text:messages[lang], url:"https://kuabo.co" });
    } else {
  ...
  const Section = ({ title }: { title:string }) => (
    <div style={{fontSize:10,color:"#555",letterSpacing:"0.12em",textTransform:"uppercase" as const,marginBottom:10,marginTop:20,paddingLeft:4}}>{title}</div>
  );

  const Row = ({ icon, label, right, onClick, danger }: { icon:string|React.ReactNode; label:string; right?:React.ReactNode; onClick?:()=>void; danger?:boolean }) => (
    <div onClick={onClick} style={{background:danger?"rgba(239,68,68,0.04)":"#141d2e",border:"1px solid "+(danger?"rgba(239,68,68,0.15)":"#1e2a3a"),borderRadius:12,padding:"13px 14px",marginBottom:6,display:"flex",alignItems:"center",justifyContent:"space-between",cursor:onClick?"pointer":"default"}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <div style={{width:32,height:32,borderRadius:8,background:"#1a2438",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>{icon}</div>
        <span style={{fontSize:14,color:danger?"#ef4444":"#fff"}}>{label}</span>
      </div>
      {right || (onClick && <ChevronRight size={16} color={danger?"#ef4444":"#555"} />)}
    </div>
  );

  const L = {
    fr:{ title:"Mon Profil", situation:"DV Lottery", phase:"Phase 1 — Installation", score:"Score d'intégration", steps:"étapes complétées", editName:"Modifier le nom", saveName:"Sauvegarder", cancelName:"Annuler", security:"Sécurité", changePass:"Changer le mot de passe", passSent:"✅ Email envoyé ! Vérifie ta boîte mail.", preferences:"Préférences", language:"Langue", privacy:"Confidentialité", commMap:"Carte communauté", commSub:"Apparaître anonymement", messages:"Messages", msgSub:"Recevoir des messages", notifications:"Notifications", notifSub:"Rappels quotidiens", share:"Partager Kuabo", shareSub:"Inviter un ami immigrant", help:"Aide", helpEmail:"support@kuabo.co", helpWhatsApp:"WhatsApp Support", helpSite:"kuabo.co", reportBug:"Signaler un bug", suggest:"Suggérer une fonctionnalité", legal:"Légal", terms:"Conditions d'utilisation", privacy2:"Politique de confidentialité", licenses:"Licences open source", account:"Compte", logout:"Déconnexion", deleteAccount:"Supprimer mon compte", deleteSub:"Action irréversible", version:"Version 1.0 · Kuabo", badges:"Mes badges", allDone:"Fully Settled 🏆" },
    en:{ title:"My Profile", situation:"DV Lottery", phase:"Phase 1 — Installation", score:"Integration score", steps:"steps completed", editName:"Edit name", saveName:"Save", cancelName:"Cancel", security:"Security", changePass:"Change password", passSent:"✅ Email sent! Check your inbox.", preferences:"Preferences", language:"Language", privacy:"Privacy", commMap:"Community map", commSub:"Appear anonymously", messages:"Messages", msgSub:"Receive messages", notifications:"Notifications", notifSub:"Daily reminders", share:"Share Kuabo", shareSub:"Invite an immigrant friend", help:"Help", helpEmail:"support@kuabo.co", helpWhatsApp:"WhatsApp Support", helpSite:"kuabo.co", reportBug:"Report a bug", suggest:"Suggest a feature", legal:"Legal", terms:"Terms of Service", privacy2:"Privacy Policy", licenses:"Open source licenses", account:"Account", logout:"Logout", deleteAccount:"Delete my account", deleteSub:"Irreversible action", version:"Version 1.0 · Kuabo", badges:"My badges", allDone:"Fully Settled 🏆" },
    es:{ title:"Mi Perfil", situation:"DV Lottery", phase:"Fase 1 — Instalación", score:"Puntuación integración", steps:"pasos completados", editName:"Editar nombre", saveName:"Guardar", cancelName:"Cancelar", security:"Seguridad", changePass:"Cambiar contraseña", passSent:"✅ ¡Email enviado! Revisa tu bandeja.", preferences:"Preferencias", language:"Idioma", privacy:"Privacidad", commMap:"Mapa comunidad", commSub:"Aparecer anónimamente", messages:"Mensajes", msgSub:"Recibir mensajes", notifications:"Notificaciones", notifSub:"Recordatorios diarios", share:"Compartir Kuabo", shareSub:"Invitar un amigo inmigrante", help:"Ayuda", helpEmail:"support@kuabo.co", helpWhatsApp:"WhatsApp Soporte", helpSite:"kuabo.co", reportBug:"Reportar un error", suggest:"Sugerir una función", legal:"Legal", terms:"Términos de Servicio", privacy2:"Política de Privacidad", licenses:"Licencias open source", account:"Cuenta", logout:"Cerrar sesión", deleteAccount:"Eliminar mi cuenta", deleteSub:"Acción irreversible", version:"Versión 1.0 · Kuabo", badges:"Mis insignias", allDone:"Fully Settled 🏆" },
  }[lang];
    ...
    <div style={{paddingBottom:20}}>
      ...
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",marginBottom:20}}>
        ...
        {editingName ? (
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
            <input
              ...
              style={{background:"#141d2e",border:"1px solid #e8b84b",borderRadius:8,padding:"6px 12px",color:"#f4f1ec",fontSize:14,fontFamily:"inherit",outline:"none"}}
              autoFocus
            />
            <button onClick={saveName} disabled={savingName} style={{background:"#e8b84b",border:"none",borderRadius:8,padding:"6px 12px",color:"#000",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
              {savingName?"...":L.saveName}
            </button>
            <button onClick={() => setEditingName(false)} style={{background:"#141d2e",border:"1px solid #1e2a3a",borderRadius:8,padding:"6px 12px",color:"#aaa",fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>
              {L.cancelName}
            </button>
          </div>
        ) : (
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
            ...
            <button onClick={() => setEditingName(true)} style={{background:"none",border:"none",cursor:"pointer",padding:4,color:"#aaa"}}>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap" as const,justifyContent:"center"}}>
          <div style={{padding:"4px 12px",borderRadius:20,background:"rgba(232,184,75,0.1)",border:"1px solid rgba(232,184,75,0.25)",fontSize:11,color:"#e8b84b",fontWeight:600}}>🎰 {L.situation}</div>
          {(userCity||userState) && <div style={{padding:"4px 12px",borderRadius:20,background:"rgba(45,212,191,0.08)",border:"1px solid rgba(45,212,191,0.2)",fontSize:11,color:"#2dd4bf",fontWeight:600}}>📍 {userCity||userState}</div>}
        </div>
      </div>

      {/* Score */}
      <div style={{background:"#141d2e",border:"1px solid #1e2a3a",borderRadius:14,padding:"16px",marginBottom:4}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap" as const,marginBottom:4}}>
        {allDone && (
          <div style={{padding:"6px 14px",borderRadius:20,background:"rgba(232,184,75,0.15)",border:"1px solid rgba(232,184,75,0.4)",fontSize:12,fontWeight:700,color:"#e8b84b"}}>
            {FULLY_SETTLED[lang]}
          </div>
        )}
        {Object.entries(BADGES).map(([id,badge]) => {
          ...
          return (
            <div key={id} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 12px",borderRadius:20,background:earned?"rgba(232,184,75,0.1)":"#141d2e",border:"1px solid "+(earned?"rgba(232,184,75,0.3)":"#1e2a3a"),opacity:earned?1:0.4}}>
              <span style={{fontSize:16}}>{badge.emoji}</span>
              <span style={{fontSize:11,color:earned?"#e8b84b":"#555",fontWeight:earned?600:400}}>{badge.label[lang]}</span>
            </div>
          );
        })}
      </div>
      ...
      <div style={{background:"#141d2e",border:"1px solid #1e2a3a",borderRadius:12,padding:"13px 14px",marginBottom:6}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:32,height:32,borderRadius:8,background:"#1a2438",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>🔐</div>
      ...
      <div style={{background:"#141d2e",border:"1px solid #1e2a3a",borderRadius:12,padding:"13px 14px",marginBottom:6}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:32,height:32,borderRadius:8,background:"#1a2438",display:"flex",alignItems:"center",justifyContent:"center"}}><Globe size={16} color="#e8b84b" /></div>
      ...
      <div style={{background:"#141d2e",border:"1px solid #1e2a3a",borderRadius:12,padding:"13px 14px",marginBottom:6}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:32,height:32,borderRadius:8,background:"#1a2438",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>🗺️</div>
            <div>
              <div style={{fontSize:13,fontWeight:500,color:"#f4f1ec"}}>{L.commMap}</div>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:32,height:32,borderRadius:8,background:"#1a2438",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>💬</div>
            <div>
              <div style={{fontSize:13,fontWeight:500,color:"#f4f1ec"}}>{L.messages}</div>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:32,height:32,borderRadius:8,background:"#1a2438",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>🔔</div>
            <div>
              <div style={{fontSize:13,fontWeight:500,color:"#f4f1ec"}}>{L.notifications}</div>
      ...
      <div onClick={handleShare} style={{background:"linear-gradient(135deg,rgba(232,184,75,0.08),rgba(45,212,191,0.05))",border:"1px solid rgba(232,184,75,0.2)",borderRadius:12,padding:"13px 14px",marginBottom:6,display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div style={{width:32,height:32,borderRadius:8,background:"rgba(232,184,75,0.12)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>👥</div>
          <div>
      ...
      <Row icon="📧" label={L.helpEmail} onClick={() => window.location.href="mailto:support@kuabo.co"} />
      <Row icon="💬" label={L.helpWhatsApp} onClick={() => window.open("https://wa.me/19705340694","_blank")} />
      <Row icon="🌐" label={L.helpSite} onClick={() => window.open("https://kuabo.co","_blank")} />
      <Row icon="🐛" label={L.reportBug} onClick={() => window.location.href="mailto:support@kuabo.co?subject=Bug Kuabo"} />
      <Row icon="💡" label={L.suggest} onClick={() => window.location.href="mailto:support@kuabo.co?subject=Suggestion Kuabo"} />

      {/* Légal */}
      <Section title={L.legal} />
      <Row icon="📄" label={L.terms} onClick={() => window.open("/terms","_blank")} />
      <Row icon="🔒" label={L.privacy2} onClick={() => window.open("/privacy","_blank")} />
      <Row icon="📜" label={L.licenses} onClick={() => window.open("https://github.com","_blank")} />

      {/* Compte */}
      <Section title={L.account} />
      <Row icon={<LogOut size={16} color="#ef4444" />} label={L.logout} onClick={onLogout} danger />
      <div onClick={onDeleteAccount} style={{background:"rgba(239,68,68,0.04)",border:"1px solid rgba(239,68,68,0.15)",borderRadius:12,padding:"13px 14px",marginBottom:6,display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div style={{width:32,height:32,borderRadius:8,background:"#1a2438",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>🗑️</div>
          <div>
      ...
      {/* Version */}
      <div style={{textAlign:"center" as const,fontSize:11,color:"#333",paddingTop:8,paddingBottom:4}}>{L.version}</div>
    </div>
  );
}
...
export default function Dashboard() {
  ...
  const [deadlineBannerStep, setDeadlineBannerStep] = useState<Step|null>(null);
  ...
  // ✅ Checklist items avant départ
  const CHECKLIST_ITEMS: Record<Lang,{id:string;label:string}[]> = {
    fr:[
      {id:"passport",   label:"Passeport valide (6 mois minimum)"},
      {id:"visa",       label:"Visa immigrant approuvé"},
      {id:"ticket",     label:"Billet d'avion confirmé"},
      {id:"cash",       label:"Argent liquide ($500 minimum)"},
      {id:"insurance",  label:"Assurance voyage souscrite"},
      {id:"contacts",   label:"Contacts d'urgence notés"},
      {id:"address",    label:"Adresse temporaire aux USA"},
      {id:"copies",     label:"Copies des documents importants"},
    ],
    en:[
      {id:"passport",   label:"Valid passport (6 months minimum)"},
      {id:"visa",       label:"Approved immigrant visa"},
      {id:"ticket",     label:"Confirmed flight ticket"},
      {id:"cash",       label:"Cash ($500 minimum)"},
      {id:"insurance",  label:"Travel insurance purchased"},
      {id:"contacts",   label:"Emergency contacts noted"},
      {id:"address",    label:"Temporary address in USA"},
      {id:"copies",     label:"Copies of important documents"},
    ],
    es:[
      {id:"passport",   label:"Pasaporte válido (6 meses mínimo)"},
      {id:"visa",       label:"Visa de inmigrante aprobada"},
      {id:"ticket",     label:"Billete de avión confirmado"},
      {id:"cash",       label:"Efectivo ($500 mínimo)"},
      {id:"insurance",  label:"Seguro de viaje contratado"},
      {id:"contacts",   label:"Contactos de emergencia anotados"},
      {id:"address",    label:"Dirección temporal en EE.UU."},
      {id:"copies",     label:"Copias de documentos importantes"},
    ],
  };
  ...
  // ✅ Notification deadline — s'affiche si étape urgente dans 3 jours
  useEffect(() => {
    const nextStep = steps.find(s => !completedSteps.includes(s.id));
    if (!nextStep) return;
    if (nextStep.urgency==="critical"||nextStep.time<=3) {
      setDeadlineBannerStep(nextStep);
      setShowDeadlineBanner(true);
      const t = setTimeout(() => setShowDeadlineBanner(false), 6000);
      return () => clearTimeout(t);
    }
  ...
  const toggleStep = async (stepId: string) => {
    ...
    try { await updateDoc(doc(db,"users",user.uid),{completedSteps:updated}); } catch { /* continue */ }
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      if (err.code==="auth/requires-recent-login") {
        ...
        setDeleteError(lang==="fr"?"Erreur — réessaie":lang==="es"?"Error":"Error — try again");
      }
      setDeleting(false);
    }
  };
  ...
  return (
    <div style={{background:"#0b0f1a",minHeight:"100dvh",color:"#fff"}}>
      ...
      {/* ✅ Bannière notification deadline */}
      {showDeadlineBanner && deadlineBannerStep && (
        <div style={{position:"fixed",top:0,left:0,right:0,zIndex:500,background:"linear-gradient(135deg,#ef4444,#f97316)",padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",animation:"slideDown 0.3s ease"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <AlertTriangle size={18} color="#fff" />
            <div>
              <div style={{fontSize:13,fontWeight:700,color:"#fff"}}>
                {lang==="fr"?`⏰ Urgent — ${deadlineBannerStep.label}`:lang==="es"?`⏰ Urgente — ${deadlineBannerStep.label}`:`⏰ Urgent — ${deadlineBannerStep.label}`}
              </div>
              <div style={{fontSize:11,color:"rgba(255,255,255,0.8)"}}>
                {lang==="fr"?"Clique pour voir le guide →":lang==="es"?"Toca para ver la guía →":"Tap to view the guide →"}
              </div>
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <button onClick={() => window.location.href=`/guide/${deadlineBannerStep.id}`} style={{background:"rgba(255,255,255,0.2)",border:"none",borderRadius:8,padding:"6px 12px",color:"#fff",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>
              {lang==="fr"?"Guide":lang==="es"?"Guía":"Guide"}
            </button>
            <button onClick={() => setShowDeadlineBanner(false)} style={{background:"none",border:"none",cursor:"pointer",color:"rgba(255,255,255,0.8)",padding:4}}>
              <X size={18} />
            </button>
          </div>
        </div>
            ...
            {deleteStep===1&&(
              <>
                ...
                <div style={{fontSize:13,color:"#aaa",textAlign:"center",lineHeight:1.7,marginBottom:24}}>
                  {lang==="fr"?"Cette action est irréversible. Toutes tes données seront supprimées définitivement.":lang==="es"?"Esta acción es irreversible. Todos tus datos serán eliminados definitivamente.":"This action is irreversible. All your data will be permanently deleted."}
                </div>
      ...
      <div style={container}>
        ...
        {activeTab==="home"&&(
          <>
            <div style={{marginTop:showDeadlineBanner?60:14}}>
            ...
            {arrival==="not-yet"&&(
              <div style={{marginTop:16,background:"rgba(232,184,75,0.06)",border:"1px solid rgba(232,184,75,0.2)",borderRadius:16,padding:"16px"}}>
                  ...
                  {CHECKLIST_ITEMS[lang].map(item => {
                    ...
                    return (
                      <div key={item.id} onClick={() => {
                        ...
                        <span style={{fontSize:13,color:checked?"#555":"#f4f1ec",textDecoration:checked?"line-through":"none",transition:"all 0.2s"}}>{item.label}</span>
                      </div>
                    );
                  })}
                </div>
                ...
                <div style={{fontSize:11,color:"#aaa",marginTop:6,textAlign:"right" as const}}>
            ...
            {completedSteps.length>0&&(
              <div style={{marginTop:16,display:"flex",gap:8,flexWrap:"wrap" as const}}>
                {Object.entries(BADGES).filter(([id]) => completedSteps.includes(id)).map(([id,badge]) => (
                  <div key={id} style={{display:"flex",alignItems:"center",gap:5,padding:"5px 10px",borderRadius:20,background:"rgba(232,184,75,0.1)",border:"1px solid rgba(232,184,75,0.25)"}}>
