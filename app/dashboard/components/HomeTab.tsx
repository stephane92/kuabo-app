      ...
      {tabs.map(({ id, icon, label })=>{
        ...
        return (
          <button key={id}
            ...
            <span style={{ fontSize:22,filter:active?"none":"grayscale(1) opacity(.45)",transition:"filter .15s" }}>{icon}</span>
            <span style={{ fontSize:10,fontWeight:active?600:400,color:active?"#e8b84b":"#4a5568",transition:"color .15s" }}>{label}</span>
          </button>
        );
      })}
    </div>
...
export default function HomeTab({ lang, userId, completedSteps, currentPhase, phaseProgress, arrivalDate, armyStatus, userState, userCity, userCountry, streak, preChecklist, userStatus, onOpenStep, onViewArmyGuide, onTogglePreChecklist }: {
  ...
  streak: number; preChecklist: Record<string,boolean>; userStatus: UserStatus;
