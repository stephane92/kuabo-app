...
function KuaboScoreWidget({ score, streak, badges, lang }: {
  ...
  return (
    <div style={{ background:"linear-gradient(135deg,#141d2e,#0f1521)", border:`1px solid ${scoreColor}30`, borderRadius:18, padding:"16px", marginBottom:14, position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ fontSize:10, color:scoreColor, fontWeight:700, letterSpacing:".1em", textTransform:"uppercase" as const, marginBottom:12 }}>
        ⭐ {L.title}
      </div>
      ...
      <div style={{ display:"flex", alignItems:"flex-end", gap:8, marginBottom:10 }}>
        ...
        <div style={{ marginLeft:"auto", fontSize:12, color:scoreColor, fontWeight:700, background:`${scoreColor}15`, padding:"4px 10px", borderRadius:10, border:`1px solid ${scoreColor}30` }}>
          {label}
        </div>
      </div>
...
function CountdownSection({ arrivalDate, lang, completedSteps, onOpenStep }: {
  ...
  const allDeadlines = [
    ...
    { id:"taxes_annual",days:365,priority:11 },{ id:"renew_greencard",days:3650,priority:14 },
  ];
    ...
    <div style={{ marginBottom:14 }}>
      ...
      {others.length>0&&(
        <div style={{ background:"#141d2e", border:"1px solid #1e2a3a", borderRadius:12, padding:"12px 14px" }}>
          ...
          {others.map((d,i)=>{ const s=allPhaseSteps.find(st=>st.id===d.id); const dc=d.daysLeft<=7?"#ef4444":d.daysLeft<=14?"#f97316":"#e8b84b"; return (
...
function DailyTip({ lang, userState, userCountry }: { lang:Lang; userState:string; userCountry:string }) {
  ...
  useEffect(()=>{ const f=async()=>{ try{ const snap=await getDocs(collection(db,"admin_messages")); let found:string|null=null; snap.forEach(d=>{ const data=d.data() as any; if(!data.active||data.type!=="conseil")return; if((data.state==="ALL"||data.state===userState)&&(data.country==="ALL"||data.country===userCountry)) found=data["text_"+lang]||data.text_fr||null; }); if(found){setTip(found);setIsAdmin(true);}else setTip(TIPS[lang][new Date().getDate()%TIPS[lang].length]); }catch{ setTip(TIPS[lang][new Date().getDate()%TIPS[lang].length]); } setLoaded(true); }; f(); },[lang,userState,userCountry]);
  return (
  ...
  return (
    <button onClick={()=>{ localStorage.setItem("completedSteps",JSON.stringify(completedSteps)); window.location.href="/chat"; }}
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12 }}>
        ...
        <div><div style={{ fontSize:15, fontWeight:600, color:"#f4f1ec" }}>{L.title}</div><div style={{ fontSize:12, color:"#aaa", marginTop:2 }}>{L.sub}</div></div>
      </div>
...
function AdminEventsAndPub({ lang, userState, userCountry, userId }: { lang:Lang; userState:string; userCountry:string; userId:string }) {
  ...
  const getPubEmoji=(title:string):string=>{ const t=(title||"").toLowerCase(); if(t.includes("bank")||t.includes("banque")||t.includes("chase"))return "🏦"; if(t.includes("phone")||t.includes("sim")||t.includes("mobile"))return "📱"; if(t.includes("job")||t.includes("emploi"))return "💼"; if(t.includes("housing")||t.includes("logement"))return "🏠"; if(t.includes("credit")||t.includes("card"))return "💳"; if(t.includes("car")||t.includes("voiture"))return "🚗"; return "📢"; };
  ...
  const pubF=pub?getPubFields(pub,lang):null; const pubEmoji=pubF?getPubEmoji(pubF.title):"📢";
  return (
    <>
      {selectedPub&&(()=>{ const f=getPubFields(selectedPub,lang); const emoji=getPubEmoji(f.title); return (
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,.92)",zIndex:600,display:"flex",alignItems:"flex-end",justifyContent:"center",backdropFilter:"blur(8px)" }} onClick={()=>setSelectedPub(null)}>
          <div style={{ background:"#0f1521",borderRadius:"22px 22px 0 0",width:"100%",maxWidth:480,maxHeight:"92vh",overflowY:"auto",animation:"slideUp .35s ease" }} onClick={e=>e.stopPropagation()}>
            {f.image?(<div style={{ width:"100%",height:200,overflow:"hidden",borderRadius:"22px 22px 0 0",position:"relative" }}><img src={f.image} alt={f.title} style={{ width:"100%",height:"100%",objectFit:"cover" }} onError={e=>(e.currentTarget.style.display="none")}/><div style={{ position:"absolute",inset:0,background:"linear-gradient(to bottom,transparent 40%,rgba(15,21,33,.9))" }}/><button onClick={()=>setSelectedPub(null)} style={{ position:"absolute",top:14,right:14,width:32,height:32,borderRadius:"50%",background:"rgba(0,0,0,.5)",border:"none",color:"#fff",fontSize:16,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center" }}>✕</button><div style={{ position:"absolute",bottom:12,left:16 }}><div style={{ fontSize:9,color:"#e8b84b",fontWeight:700,letterSpacing:".08em",background:"rgba(0,0,0,.6)",padding:"3px 8px",borderRadius:6 }}>PARTENAIRE KUABO</div></div></div>):(<div style={{ padding:"20px 20px 0",display:"flex",alignItems:"center",justifyContent:"space-between" }}><div style={{ fontSize:9,color:"#e8b84b",fontWeight:700,letterSpacing:".08em" }}>PARTENAIRE KUABO</div><button onClick={()=>setSelectedPub(null)} style={{ background:"none",border:"none",color:"#555",fontSize:20,cursor:"pointer" }}>✕</button></div>)}
            <div style={{ padding:"20px" }}>
              <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:14 }}><div style={{ width:52,height:52,borderRadius:14,background:"rgba(232,184,75,.12)",border:"1px solid rgba(232,184,75,.25)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,flexShrink:0 }}>{emoji}</div><div><div style={{ fontSize:18,fontWeight:800,color:"#f4f1ec" }}>{f.title}</div>{f.desc&&<div style={{ fontSize:13,color:"#aaa",marginTop:3,lineHeight:1.6 }}>{f.desc}</div>}</div></div>
              {f.url&&<button onClick={()=>window.open(f.url,"_blank")} style={{ width:"100%",padding:"15px",background:"#e8b84b",border:"none",borderRadius:14,color:"#000",fontSize:15,fontWeight:800,cursor:"pointer",fontFamily:"inherit",marginBottom:10 }}>{f.cta||"Voir l'offre →"}</button>}
              <button onClick={()=>setSelectedPub(null)} style={{ width:"100%",padding:"12px",background:"transparent",border:"1px solid #1e2a3a",borderRadius:12,color:"#555",fontSize:13,cursor:"pointer",fontFamily:"inherit" }}>{lang==="fr"?"Fermer":"Close"}</button>
            </div>
          </div>
        </div>
      ); })()}

      {selectedEvt&&(()=>{ const f=getEvtFields(selectedEvt,lang); const isIn=participating[selectedEvt.id]; const dateStr=selectedEvt.eventDate?new Date(selectedEvt.eventDate).toLocaleDateString(lang==="fr"?"fr-FR":lang==="es"?"es-ES":"en-US",{weekday:"long",day:"numeric",month:"long",year:"numeric"}):""; return (
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,.92)",zIndex:600,display:"flex",alignItems:"flex-end",justifyContent:"center",backdropFilter:"blur(8px)" }} onClick={()=>setSelectedEvt(null)}>
          <div style={{ background:"#0f1521",borderRadius:"22px 22px 0 0",width:"100%",maxWidth:480,maxHeight:"92vh",overflowY:"auto",animation:"slideUp .35s ease" }} onClick={e=>e.stopPropagation()}>
            {f.image?(<div style={{ width:"100%",height:200,overflow:"hidden",borderRadius:"22px 22px 0 0",position:"relative" }}><img src={f.image} alt={f.title} style={{ width:"100%",height:"100%",objectFit:"cover" }} onError={e=>(e.currentTarget.style.display="none")}/><div style={{ position:"absolute",inset:0,background:"linear-gradient(to bottom,transparent 40%,rgba(15,21,33,.9))" }}/><button onClick={()=>setSelectedEvt(null)} style={{ position:"absolute",top:14,right:14,width:32,height:32,borderRadius:"50%",background:"rgba(0,0,0,.5)",border:"none",color:"#fff",fontSize:16,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center" }}>✕</button></div>):(<div style={{ padding:"20px 20px 0",display:"flex",justifyContent:"flex-end" }}><button onClick={()=>setSelectedEvt(null)} style={{ background:"none",border:"none",color:"#555",fontSize:20,cursor:"pointer" }}>✕</button></div>)}
            <div style={{ padding:"20px" }}>
              ...
              <div style={{ display:"flex",flexDirection:"column" as const,gap:8,marginBottom:16 }}>
                {dateStr&&(<div style={{ display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:"rgba(45,212,191,.06)",border:"1px solid rgba(45,212,191,.2)",borderRadius:10 }}><span style={{ fontSize:18 }}>📅</span><div><div style={{ fontSize:13,fontWeight:600,color:"#2dd4bf" }}>{dateStr}</div>{selectedEvt.eventTime&&<div style={{ fontSize:11,color:"#aaa" }}>{selectedEvt.eventTime}</div>}</div></div>)}
                {selectedEvt.eventLocation&&(<div style={{ display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:"rgba(167,139,250,.06)",border:"1px solid rgba(167,139,250,.2)",borderRadius:10 }}><span style={{ fontSize:18 }}>📍</span><div style={{ flex:1 }}><div style={{ fontSize:13,fontWeight:600,color:"#a78bfa" }}>{selectedEvt.eventLocation}</div></div><button onClick={()=>window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(selectedEvt.eventLocation)}`,"_blank")} style={{ padding:"5px 10px",background:"rgba(167,139,250,.1)",border:"1px solid rgba(167,139,250,.3)",borderRadius:8,color:"#a78bfa",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"inherit" }}>Maps →</button></div>)}
                <div style={{ display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:"#141d2e",border:"1px solid #1e2a3a",borderRadius:10 }}><span style={{ fontSize:18 }}>👥</span><span style={{ fontSize:13,color:"#aaa" }}>{(selectedEvt.participants||[]).length} participants</span></div>
              </div>
              {f.desc&&<div style={{ fontSize:13,color:"#aaa",lineHeight:1.7,marginBottom:16 }}>{f.desc}</div>}
              <button onClick={()=>{handleParticipate(selectedEvt.id);setSelectedEvt(null);}} style={{ width:"100%",padding:"15px",background:isIn?"rgba(239,68,68,.1)":"#2dd4bf",border:isIn?"1px solid rgba(239,68,68,.3)":"none",borderRadius:14,color:isIn?"#ef4444":"#000",fontSize:15,fontWeight:800,cursor:"pointer",fontFamily:"inherit",marginBottom:10 }}>{isIn?(lang==="fr"?"❌ Se désinscrire":"❌ Cancel"):(lang==="fr"?"✅ Je participe !":"✅ I'm in!")}</button>
              <button onClick={()=>setSelectedEvt(null)} style={{ width:"100%",padding:"12px",background:"transparent",border:"1px solid #1e2a3a",borderRadius:12,color:"#555",fontSize:13,cursor:"pointer",fontFamily:"inherit" }}>{lang==="fr"?"Fermer":"Close"}</button>
            </div>
          </div>
        </div>
      ); })()}
      ...
      {pub&&pubF&&!pubClosed&&(
        <div style={{ marginBottom:14 }}>
          ...
          <div style={{ position:"relative",background:"linear-gradient(135deg,#141d2e,#0f1521)",border:"1px solid rgba(232,184,75,0.2)",borderRadius:14,overflow:"hidden",cursor:"pointer" }} onClick={()=>setSelectedPub(pub)}>
            <div style={{ height:2,background:"linear-gradient(to right,#e8b84b,#f97316)" }}/>
            {pubF.image&&(<div style={{ width:"100%",height:120,overflow:"hidden",position:"relative" }}><img src={pubF.image} alt={pubF.title} style={{ width:"100%",height:"100%",objectFit:"cover" }} onError={e=>(e.currentTarget.style.display="none")}/><div style={{ position:"absolute",inset:0,background:"linear-gradient(to bottom,transparent 30%,rgba(20,29,46,.95))" }}/></div>)}
            <div style={{ padding:"12px 14px",display:"flex",alignItems:"center",gap:12 }}>
              ...
              <div style={{ width:46,height:46,borderRadius:13,background:"rgba(232,184,75,.12)",border:"1px solid rgba(232,184,75,.25)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0 }}>{pubEmoji}</div>
        ...
        <div style={{ marginBottom:14 }}>
          ...
          {events.map(event=>{ const ef=getEvtFields(event,lang); const isIn=participating[event.id]; const dateStr=event.eventDate?new Date(event.eventDate).toLocaleDateString(lang==="fr"?"fr-FR":"en-US",{day:"numeric",month:"long"}):""; return (
            <div key={event.id} style={{ background:"#141d2e",border:"1px solid rgba(45,212,191,0.2)",borderRadius:14,overflow:"hidden",position:"relative",marginBottom:8,cursor:"pointer" }} onClick={()=>setSelectedEvt(event)}>
              ...
              {ef.image&&(<div style={{ width:"100%",height:100,overflow:"hidden",position:"relative" }}><img src={ef.image} alt={ef.title} style={{ width:"100%",height:"100%",objectFit:"cover" }} onError={e=>(e.currentTarget.style.display="none")}/><div style={{ position:"absolute",inset:0,background:"linear-gradient(to bottom,transparent 30%,rgba(20,29,46,.9))" }}/></div>)}
...
// ══════════════════════════════════════════════
export default function HomeTab({ lang, userId, completedSteps, currentPhase, phaseProgress, arrivalDate, armyStatus, userState, userCity, userCountry, streak, userStatus, arrivalConfirmed, onOpenStep, onViewArmyGuide }: {
  ...
  return (
    <>
      ...
      {/* 8. Pub + Événements Admin */}
      {userId&&<AdminEventsAndPub lang={lang} userState={userState} userCountry={userCountry} userId={userId}/>}

      {/* 9. Parcours */}
      <div style={{ marginBottom:14 }}>
        <div style={{ fontSize:11,color:"#555",letterSpacing:".1em",textTransform:"uppercase" as const,marginBottom:12,fontWeight:600 }}>🗺️ {lang==="fr"?"Ton parcours Kuabo":lang==="es"?"Tu camino Kuabo":"Your Kuabo Journey"}</div>
